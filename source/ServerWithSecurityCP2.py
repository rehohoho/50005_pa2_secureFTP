...
def get_signed_crt():
    ...
    with open("./auth/server_signed.crt", mode="rb") as f:
        crt_encrypted_data = f.read()
    return crt_encrypted_data


def get_session_key(client_socket, private_key):
    key_len = convert_bytes_to_int(read_bytes(client_socket, 8))
    ...
    return Fernet(session_key_bytes)
                ...
                while True:
                    match convert_bytes_to_int(read_bytes(client_socket, 8)):
                        ...
                        case 1:
                            ...
                            data_len = convert_bytes_to_int(
                                read_bytes(client_socket, 8)
                            )

                            encrypted_data = read_bytes(client_socket, data_len)
                        ...
                        case 3:
                            ...
                            crt_encrypted_data = get_signed_crt()
                            ...
                            client_socket.sendall(
                                convert_int_to_bytes(len(crt_encrypted_data))
                            )
                            client_socket.sendall(crt_encrypted_data)
                        case 4:
                            session_key = get_session_key(client_socket, get_server_private_key())
